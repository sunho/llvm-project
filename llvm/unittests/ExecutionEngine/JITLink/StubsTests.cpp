...
static std::pair<Symbol&, Symbol&> GenerateStub(LinkGraph& G, size_t PointerSize, Edge::Kind PointerEdgeKind) {
  ...
  auto PointerSym = createAnonymousPointer(G, PointersSec, &FuncSymbol);
  EXPECT_FALSE(errorToBool(PointerSym.takeError()));
  EXPECT_EQ(std::distance(PointerSym->getBlock().edges().begin(), PointerSym->getBlock().edges().end()), 1U);
  ...
  EXPECT_TRUE(all_of(PointerSym->getBlock().getContent(), [](char x) { return x == 0; }));

  auto StubSym = createAnonymousPointerJumpStub(G, StubsSec, *PointerSym);
...
TEST(StubsTest, StubsGeneration_x86_64) {
  ...
  EXPECT_EQ(std::distance(StubSym.getBlock().edges().begin(), StubSym.getBlock().edges().end()), 1U);
  ...
  EXPECT_EQ(StubSym.getBlock().getContent(), ArrayRef<char>(PointerJumpStubContent));
}
...
TEST(StubsTest, StubsGeneration_aarch64) {
  ...
  EXPECT_EQ(std::distance(StubSym.getBlock().edges().begin(), StubSym.getBlock().edges().end()), 2U);
...
TEST(StubsTest, StubsGeneration_loongarch32) {
  const char PointerJumpStubContent[12] = {
    ...
    static_cast<char>(0x94), 0x02, static_cast<char>(0x80), 0x28, // ld.d $t8, $t8, %pageoff12(imm)
    static_cast<char>(0x80), 0x02, 0x00, 0x4c  // jr $t8
  };
...
TEST(StubsTest, StubsGeneration_loongarch64) {
  const char PointerJumpStubContent[12] = {
    0x14, 0x00, 0x00, 0x1a, // pcalau12i $t8, %page20(imm)
    static_cast<char>(0x94), 0x02, static_cast<char>(0xc0), 0x28, // ld.d $t8, $t8, %pageoff12(imm)
    static_cast<char>(0x80), 0x02, 0x00, 0x4c  // jr $t8
  };
