...
protected:
  void SetUp() override {
    ...
    auto CompileFunction = std::make_unique<TMOwningSimpleCompiler>(std::move(*TM));
...
static Function* createRetFunction(Module* M, StringRef Name, uint32_t ReturnCode) {
  Function *Result =
      Function::Create(FunctionType::get(Type::getInt32Ty(M->getContext()), {}, false), GlobalValue::ExternalLinkage, Name, M);
...
TEST_F(ReOptimizeLayerTest, BasicReOptimization) {
  ...
  auto RM = JITLinkRedirectableSymbolManager::Create(*ES, *ObjLinkingLayer, *JD);
  EXPECT_THAT_ERROR(RM.takeError(), Succeeded());

  ROLayer = std::make_unique<ReOptimizeLayer>(*ES, *CompileLayer, **RM);
  ROLayer->setReoptimizeFunc([&](ReOptimizeLayer& Parent, ReOptMaterializationUnitID MUID, unsigned CurVerison, ResourceTrackerSP OldRT, ThreadSafeModule& TSM) {
    TSM.withModuleDo([&](Module& M) {
      for (auto& F : M) {
        ...
        for (auto& B : F) {
          for (auto& I : B) {
            if (ReturnInst* Ret = dyn_cast<ReturnInst>(&I)) {
              Value *RetValue = ConstantInt::get(M.getContext(), APInt(32, 53));
  ...
  EXPECT_THAT_ERROR(
    addIRModule(JD->getDefaultResourceTracker(), ThreadSafeModule(std::move(M), std::move(Ctx))),
      Succeeded());
