...
std::optional<uint32_t> Lexer::tryReadNumericUCN(unsigned &StartOffset,
  ...
  if (Delimited && Kind == 'U') {
    if (Diagnose)
      Diag(SlashLoc, diag::err_hex_escape_no_digits) << StringRef(BufferStart+KindLoc, 1);
    return llvm::None;
  }
...
llvm::Optional<uint32_t> Lexer::tryReadNamedUCN(unsigned &StartOffset,
  ...
  if (!Match) {
    ...
    if (Diagnose) {
      Diag(StartOffset, diag::err_invalid_ucn_name)
          ...
          << makeCharRange(*this, StartOffset, CurPtr - CharSize);
      if (LooseMatch) {
  ...
  if (Result) {
    ...
    // we need to call getAndAdvanceChar again to set the appropriate flags
    // on Result.
    if (CurOffset - StartOffset == (ptrdiff_t)(Buffer.size() + ))
      StartOffset = CurOffset;
    else
      while (StartOffset != CurOffset)
        (void)getAndAdvanceChar(StartOffset, *Result);
